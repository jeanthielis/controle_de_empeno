...
createApp({
    setup() {
        ...
        const navigateAdmin = (tab) => { adminTab.value = tab; };
        ...
        const removerInspecao = async (id) => {
            if(confirm('Tem certeza que deseja EXCLUIR este registro?')) {
                try {
                    ...
                    notify('Excluído', 'Registro removido com sucesso.', 'sucesso');
                } catch(e) {
                    notify('Erro', 'Erro ao excluir.', 'erro');
                }
            }
        };
        ...
        const handleLogin = () => {
            ...
            setTimeout(() => {
                ...
                if (userLower === 'admin' && pass === 'admin') { currentView.value = 'admin'; notify('Super Admin', 'OK', 'sucesso'); loading.value = false; return; }
                const usuarioEncontrado = cadastros.value.usuarios.find(u => u.login === userLower && u.matricula === pass);
                if (usuarioEncontrado) {
                    if (remember) { localStorage.setItem('qc_user', userLower); localStorage.setItem('qc_pass', pass); } else { localStorage.removeItem('qc_user'); localStorage.removeItem('qc_pass'); }
                    if (usuarioEncontrado.admin) { currentView.value = 'admin'; } else { currentView.value = 'inspector'; if(form.value.pecas.length === 0) adicionarPeca(); }
                    notify('Bem-vindo', `Olá, ${usuarioEncontrado.nome}`, 'sucesso');
                } else { notify('Erro', 'Incorreto.', 'erro'); }
                loading.value = false;
            }, 600);
        };
        ...
        const cadastrarUsuario = async () => {
            ...
            try { await addDoc(collection(db, "usuarios"), { nome: nome, matricula: matricula, login: loginFinal, admin: admin }); novoUsuarioForm.value = { nome: '', matricula: '', admin: false }; notify('Sucesso', `Login: ${loginFinal}`, 'sucesso'); } catch (e) { notify('Erro', e.message, 'erro'); }
        };

        const mascararInput = (event, pecaObj, tipo, chave) => {
            let input = event.target; let valorOriginal = input.value; let isNegative = valorOriginal.includes('-'); let numeros = valorOriginal.replace(/\D/g, ''); let digitosReais = numeros.replace(/^0+/, ''); let valorVisual = ''; let valorFloat = 0;
            ...
            if (numeros.length === 0 && !isNegative) { valorVisual = ''; valorFloat = null; } else if (numeros.length === 0 && isNegative) { valorVisual = '-'; }
            if (tipo === 'laterais') { pecaObj.lateraisDisplay[chave] = valorVisual; pecaObj.laterais[chave] = valorFloat; } else { pecaObj.centraisDisplay[chave] = valorVisual; pecaObj.centrais[chave] = valorFloat; }
            input.value = valorVisual; if (digitosReais.length >= 3) focarProximoInput(input); salvarRascunho();
        };
        ...
        const salvarRascunho = async () => {
            ...
            form.value.pecas.forEach(p => { Object.values(p.laterais).forEach(v => { if(getStatusClass(v, 'lateral') === 'status-bad') resultadoGeral = 'Reprovado'; }); Object.values(p.centrais).forEach(v => { if(getStatusClass(v, 'central') === 'status-bad') resultadoGeral = 'Reprovado'; }); });
        ...
        const gerarRelatorioFinal = async () => {
            ...
            if (!form.value.posFolga) { notify('Atenção', 'Preencha Pós Folga.', 'erro'); return; }
        ...
        return {
            ...
            adminTab, navigateAdmin, cadastros, filtros, relatoriosFiltrados, limparFiltros,
